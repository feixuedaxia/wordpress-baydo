...
class RatingHandler
{
    ...
    public function dismiss($slug, $force)
    {
        ...
        if (\MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::set($initiator, \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING, $ts)) {
            return $ts;
        }
        return \false;
    }
    ...
    public function getCanBeRated()
    {
        ...
        foreach (\MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getInitiators() as $initiator) {
            $ts = \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::get($initiator, \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING);
            // Initialize default
            if ($ts === null) {
                $ts = $initiator->getNextRatingPopup(\true);
                \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::set($initiator, \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\TransientHandler::TRANSIENT_NEXT_RATING, $ts);
            }
