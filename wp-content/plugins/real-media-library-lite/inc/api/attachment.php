...
if (!\function_exists('wp_attachment_folder')) {
    ...
    {
        return \MatthiasWeb\RealMediaLibrary\attachment\Filter::getInstance()->getAttachmentFolder($attachmentId, $default);
    }
}
if (!\function_exists('wp_attachment_order_update')) {
    ...
    function wp_attachment_order_update($folderId, $attachmentId, $nextId, $lastIdInView = \false)
    {
        ...
        if (\is_rml_folder($folder)) {
            ...
            \MatthiasWeb\RealMediaLibrary\Core::getInstance()->debug("Could not find the folder with id {$folderId}", __FUNCTION__);
...
if (!\function_exists('wp_rml_move')) {
    ...
    function wp_rml_move($to, $ids, $supress_validation = \false, $isShortcut = \false)
    {
        ...
        if (\is_rml_folder($folder)) {
            ...
            \MatthiasWeb\RealMediaLibrary\Core::getInstance()->debug("Could not find the folder with id {$to}", __FUNCTION__);
            return [\__('The given folder was not found.', \RML_TD)];
        }
    }
}
